...
def update_bakery(id):
    ...
    if bakery:
        ...
        if new_name:
            ...
            return jsonify({"message": "New bakery name not provided"}), 400
    else:
        return jsonify({"message": "Bakery not found"}), 404
...
def create_baked_good():
    ...
    existing_baked_good = BakedGood.query.filter_by(name=name).first()

    if existing_baked_good:
        return jsonify({"message": "Baked good with this name already exists"}), 409
